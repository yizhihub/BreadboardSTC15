...
FMSTR_BCHR pcm_pCommBuffer[FMSTR_COMM_BUFFER_SIZE+3];    
...
static FMSTR_SERIAL_FLAGS data pcm_wFlags;
...
static FMSTR_BPTR  data pcm_pTxBuff;     /* pointer to next byte to transmit */
static FMSTR_BPTR  data pcm_pRxBuff;     /* pointer to next free place in RX buffer */
...
FMSTR_BOOL FMSTR_Rx(FMSTR_BCHR nRxChar)
{
    ...
    if(pflg->flg.bRxLastCharSOB)
    {
        ...
        *pcm_pRxBuff++ = nRxChar;
        ...
        /* yizhi define report data from another freemaster*/
        else if (nRxChar == 0x00)
        {
